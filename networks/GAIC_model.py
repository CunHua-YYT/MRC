...
import warnings
from networks.deeplabv3plus import MyModel, MyModel_shufflenet, MyModel_mobile
...
class ResizeModule(nn.Module):
    def __init__(self):
        ...
        self.upsample = nn.Upsample(size=(9, 9), mode='bilinear', align_corners=False)
...
class AttentionModule(nn.Module):
    def __init__(self, in_channels, hidden_units):
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Linear(hidden_units, in_channels),
            nn.Sigmoid()
        )

    def forward(self, x):
        batch_size, channels, _, _ = x.size()
        attention = self.pool(x).view(batch_size, channels)
        attention = self.fc(attention).view(batch_size, channels, 1, 1)
        return x * attention
...
class crop_model_multi_scale_shared(nn.Module):
    ...
    def __init__(self, alignsize = 8, reddim = 32, loadweight = True, model = None, ):
        ...
        if model == 'shufflenetv2':
            ...
            self.Feat_ext = vgg_base(loadweight)
            self.DimRed = nn.Conv2d(2048, reddim, kernel_size=1, padding=0)
        elif model == 'resnet50':
            self.Feat_ext = resnet50_base(loadweight)
        ...
        self.RoIAlign = RoIAlignAvg(alignsize, alignsize, 1.0/2**downsample)
        self.RoDAlign = RoDAlignAvg(alignsize, alignsize, 1.0/2**downsample)
        self.FC_layers = fc_layers(reddim*3, alignsize)
        self.deepModel = MyModel()
        self.attention1 = AttentionModule(2048, 16)  # 实例化注意力模块
        self.attention2 = AttentionModule(reddim * 3, 16)  # 实例化注意力模块
        self.resizeModel = ResizeModule()
    ...
    def forward(self, im_data, boxes):
        ...
        deepfeat = self.deepModel(im_data)
        resizefeat = self.resizeModel(im_data)
        ...
        cat_feat = torch.cat((f3,f4,0.5*f5,deepfeat),1)
        cat_feat = self.attention1(cat_feat)

        red_feat = self.DimRed(cat_feat)
        RoI_feat = self.RoIAlign(red_feat, boxes)
        RoD_feat = self.RoDAlign(red_feat, boxes)
        size0 = int(RoD_feat.size()[0])
        resizefeat = resizefeat.repeat(size0,1,1,1)

        final_feat = torch.cat((RoI_feat, RoD_feat,resizefeat), 1)
        final_feat = self.attention2(final_feat)  # 使用注意力模块
        prediction = self.FC_layers(final_feat)
        return prediction
