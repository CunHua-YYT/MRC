...
def parse_args():
    ...
    parser.add_argument('--image_dir', type=str,
                        default='/home/yyt/PycharmProjects/examples/GAIC-Pytorch-main/test_images',
                        help='the directory of test images')
    parser.add_argument('--save_dir', type=str, default='/home/yyt/PycharmProjects/examples/GAIC-Pytorch-main/txtResult',
...
def build_network(backbone):
    ...
    weights_path = '/home/yyt/PycharmProjects/examples/GAIC-Pytorch-main/pretrained_models/MRCVggBase.pth'.format(
        backbone, reddim)
...
def predict_best_crop(model, im_tensor, anchors, im):
    ...
    #     pr_bboxx = rescale_anchors[i].tolist()
    #     x1, y1, x2, y2 = pr_bboxx
    #     print(pr_bboxx,scores[i])
    # pr_crop = np.asarray(pr_crop)[:,:,::-1] # convert to opencv format
    return pr_crop, pr_bbox  # 剪裁图片 and 坐标


def predict_best_crop_1(model, im_tensor, anchors, im):
    im_width, im_height = im.size
    with torch.no_grad():
        rois = anchors.astype(np.float32)
        rois = torch.from_numpy(rois).unsqueeze(0).to(im_tensor.device)
    ...
    sorted_pr_bbox = rescale_anchors[sorted_indices]

    file_name,file_extension = os.path.splitext(im_name)
    file_name_only = file_name
    with open(f'/home/yyt/PycharmProjects/examples/GAIC-Pytorch-main/a/{file_name_only}.txt', 'w') as file:
        for i in range(length):
            pr_bboxx= sorted_pr_bbox[i].tolist()
            # pr_bboxx = rescale_anchors[i].tolist()
            y1, x1, y2, x2 = pr_bboxx
            # file.write(f"{pr_bboxx} {scores[i]}\n")
            file.write(f"{pr_bboxx[1]} {pr_bboxx[0]} {pr_bboxx[3]} {pr_bboxx[2]} {sorted_scores[i]}\n")
            print(pr_bboxx,scores[i])
        # pr_crop = np.asarray(pr_crop)[:,:,::-1] # convert to opencv format
    return pr_crop, pr_bbox  # 剪裁图片 and 坐标
...
if __name__ == '__main__':
    ...
    for i, img in enumerate(img_list):
        ...
        crop_1_1, bbox_1_1 = predict_best_crop_1(net, src_tensor, anchors_1_1, src)
        # print('aspect_ratio=1:1, num_candidates:{}, best crop bbox:{}, crop(w,h):{}'.format(
        #     anchors_1_1.shape[0], bbox_1_1, crop_1_1.size),im_name)
        print(im_name, bbox_1_1)
